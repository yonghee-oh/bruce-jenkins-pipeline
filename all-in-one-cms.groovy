...
def getNginxAction(nginxAction) {
    def action
    switch(nginxAction.trim().toLowerCase()) {        
        case 'start': 
            action = 'bash -ci ngstart || exit 0'
            break
        case 'restart': 
            action = 'bash -ci ngreload || exit 0'
            break
        case 'stop': 
            action = 'bash -ci ngstop || exit 0'
            break
        case 'status': 
            action = 'bash -ci ngstatus || exit 0'
            break
        default: 
            action = 'whoami'
...
def getTomcatAction(tomcatAction) {
    def action
    switch(tomcatAction.trim().toLowerCase()) {        
        case 'start': 
            action = 'bash -ci cmsboot || exit 0'
            break
        case 'restart': 
            action = 'bash -ci cmsdown || exit 0; bash -ci cmsboot || exit 0;'
            break
        case 'stop': 
            action = 'bash -ci cmsdown || exit 0'
            break
        case 'status': 
            action = 'ps -ef | grep tomcat | grep jeus'
            break
        default: 
            action = 'whoami'
            println "The value is unknown"
...
void doSaveFinalSyncVersionByRsync(remoteUserAuth, serverList, appPath, appDocBase, ownerName='') {
    withCredentials([usernamePassword(credentialsId: "$remoteUserAuth",
        ...
        def deleteTime = new Date().minus(70).format("yyyyMM")
        ...
        for (int i = 0; i < serverList.size(); i++) {
            ...
            // @Rsync:변경 부분 배포
            sshCommand remote: remote, command: "sudo su -l $owner -c 'rm -rf ~/backup/${appDocBase}.tar.gz.${deleteTime}*'", sudo: true
...
def doSequentialStageGroup(Map args) {
    ...
    stage(labelPrefix+"nginx STOP") {
        ...
        when(args.stageArgs.nginxDo && !skipStage) {
            ...
            def ngAction = getNginxAction('STOP')
    ...
    stage(labelPrefix+"tomcat STOP") {
        ...
        when(args.stageArgs.tomcatDo && !skipStage) {
            ...
            def tcAction = getTomcatAction('STOP')
    ...
    stage(labelPrefix+"sync") {
        ...
        if(args.stageArgs.deployDo && !(args.stageArgs.rsyncDeployDo || args.stageArgs.warDeployDo)) {
            println "● No Matched sync type... (NOT WORKING)"
        }
        when(args.stageArgs.deployDo && (args.stageArgs.rsyncDeployDo || args.stageArgs.warDeployDo) && !skipStage) {
    ...
    stage(labelPrefix+"tomcat START") {
        ...
        when(args.stageArgs.tomcatDo && !skipStage) {
            ...
            def tcAction = getTomcatAction('START')
            doServerAction(args.stageArgs.remoteUserAuth, args.serverList, tcAction, args.stageArgs.ownerName)
            sleep 8
    ...
    stage(labelPrefix+"nginx START") {
        ...
        when(args.stageArgs.nginxDo && !skipStage) {
            ...
            def ngAction = getNginxAction('START')
            doServerAction(args.stageArgs.remoteUserAuth, args.serverList, ngAction, args.stageArgs.ownerName)
